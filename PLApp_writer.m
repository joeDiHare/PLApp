function [statuses]=PLApp_writer()
...
for n=1:length(file_dir)
    if length(file_dir(n).name)>3
        if strcmp(file_dir(n).name(end-3:end),'.xls')
            if ~strcmp(file_dir(n).name(:),'file_plapp.xls')
                m=m+1; xls_name{m}=file_dir(n).name; 
            end
        end
    end
end

for n=1:length(xls_name)
    statuses = produce_plapp(path_d, xls_name{n});
    isOK(statuses);
end

end %EoF

function [statuses]=produce_plapp(path_d, filename)
...
source_fld = 'C:\Users\sc04\Documents\MATLAB\container-plapp\PlayListApp';
...
for n=1:length(txt(9:end,2))
    if ( ~isempty(txt{9+n-1,2}) )
        tot_sng = tot_sng + 1;
    end
end

% read date in
date_in = regexprep(lower(txt{6,3}),{'[a-z]' ':'},'');
tmp = strfind(date_in,'/');
day = str2num(date_in(1:tmp(1)-1));
mon = str2num(date_in(tmp(1)+1:tmp(2)-1));
yea = str2num(date_in(end-1:end));
%%% add date checks
if day>maxmon(mon), errordlg('wrong day of the month','wrong day of the month'); return; end
if mon>12,          errordlg('wrong month','wrong month'); return; end
if yea<15,          errordlg('wrong year','wrong year'); return; end
...
for n=1:tot_sng
%     IDs_sng(n)=num(n+2,2);
    Lnk_sng_tmp=txt{n+8,2};
    Msg_txt_tmp=txt{n+8,3};
...
message   = [' ' 10 'Hello' 10 ' ' 10 ...
             ...
             ' ' 10 ...
             'ps: for CBUers, the VPN may prevent external links from opening in the app.' 10 ' ' 10 ...
             '-Plapp Team'];
...
status = movefile(filename,strcat('C:\Users\sc04\Documents\MATLAB\container-plapp\sent_xls\',filename),'f'); isOK(status); pause(.1);

%% write it in the log_book.txt

fid_log_book = fopen(strcat(path_d,'\','plapp_log_book','.txt'),'a');

date=datestr(now,'hh/mm/ss'); time=datestr(now,'hh:mm:ss');
...
% "C:\Program Files\MATLAB\R2013a\bin\matlab.exe" -nodisplay -nosplash -nodesktop -r "run('C:\Users\sc04\Documents\MATLAB\container-plapp\PLApp_writer.m');exit;"
